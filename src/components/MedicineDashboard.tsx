...
export default function MedicineDashboard({ user, onNavigate, onStartPracticeSession, onContinuePractice, onStartMockExam, onLogout }: MedicineDashboardProps) {
  ...
  const systemIconMap: Record<string, any> = {
    ...
    'Clinical Skills': Heart,
    'Ethics & Law': Plus,
    'Preventive Medicine': Target,
    'Radiology': Activity,
    'Laboratory Medicine': Heart,
    'Pathology': Plus,
    'Internal Medicine Board Review': Target
  };
  ...
  // ✅ FIND the existing getMockExamProgress function and REPLACE it entirely:
  ...
  const getExamStatusBadge = (examName: string) => {
    ...
    if (progress?.completed) {
      return <Badge variant="default" className="text-emerald-700 bg-emerald-100"><Star className="w-3 h-3 mr-1" />Completed</Badge>;
    }
    ...
    return <Badge variant="outline" className="text-emerald-700 bg-emerald-50"><Unlock className="w-3 h-3 mr-1" />Available</Badge>;
  };
  ...
  const loadMockExamSets = async () => {
    try {
      const sets = await getMockExamSets('medicine');
  ...
  const loadMedicalSystems = async () => {
    try {
      ...
      // Get all medical systems for medicine specialty with error handling
      let allSystems;
      try {
        allSystems = getMedicalSystems('medicine');
      ...
      for (const system of allSystems) {
        ...
        try {
          realQuestionCount = await getSystemQuestionCount(system.name, 'medicine');
          ...
          const systemData = {
            ...
            icon: systemIconMap[system.name] || Activity,
  ...
  const getSystemDescription = (systemName: string): string => {
    const descriptions: Record<string, string> = {
      'Cardiovascular System': 'Heart diseases, hypertension, arrhythmias, heart failure, coronary syndromes',
      'Respiratory System': 'Asthma, COPD, pneumonia, pulmonary embolism, respiratory failure',
      'Gastroenterology': 'GI bleeding, IBD, hepatitis, peptic ulcer disease, pancreatitis',
      'Endocrine System': 'Diabetes, thyroid disorders, adrenal diseases, electrolyte imbalances',
      'Nephrology': 'AKI, CKD, glomerulonephritis, electrolyte disorders, hypertension',
      'Hematology': 'Anemia, bleeding disorders, thrombosis, hematologic malignancies',
      'Infectious Diseases': 'Sepsis, pneumonia, UTI, meningitis, tropical diseases, antibiotics',
      'Rheumatology': 'RA, SLE, vasculitis, crystal arthropathies, inflammatory disorders',
      'Neurology': 'Stroke, seizures, dementia, movement disorders, neuropathies',
      'Psychiatry': 'Depression, anxiety, psychosis, substance abuse, psychiatric emergencies',
      'Emergency Medicine': 'Trauma, poisoning, emergency procedures, resuscitation, triage',
      'Critical Care': 'Shock, mechanical ventilation, sedation, vasopressors, monitoring',
      'Dermatology': 'Skin cancers, inflammatory conditions, infections, drug reactions',
      'Ophthalmology': 'Diabetic retinopathy, glaucoma, cataracts, emergency eye conditions',
      'ENT': 'Hearing loss, sinusitis, throat infections, emergency ENT conditions',
      'Geriatrics': 'Aging, dementia, falls, polypharmacy, elderly care principles',
      'Pharmacology': 'Drug interactions, adverse effects, dosing, contraindications',
      'Clinical Skills': 'Physical examination, diagnostic reasoning, communication skills',
      'Ethics & Law': 'Medical ethics, consent, confidentiality, legal aspects of medicine',
      'Preventive Medicine': 'Screening, vaccination, health promotion, epidemiology',
      'Radiology': 'X-ray interpretation, CT, MRI, ultrasound, nuclear medicine',
      'Laboratory Medicine': 'Lab value interpretation, diagnostic tests, biochemistry',
      'Pathology': 'Disease mechanisms, pathophysiology, histopathology basics',
      'Internal Medicine Board Review': 'High-yield topics, exam-style questions, board preparation'
    };
    
    return descriptions[systemName] || 'Comprehensive medical system coverage';
  };

  const getSystemDifficulty = (systemName: string): string => {
    const highYield = ['Cardiovascular System', 'Respiratory System', 'Endocrine System', 'Nephrology', 'Infectious Diseases', 'Neurology', 'Emergency Medicine', 'Critical Care', 'Pharmacology', 'Clinical Skills', 'Internal Medicine Board Review'];
    const lowYield = ['Dermatology', 'Ophthalmology', 'ENT'];
  ...
  const getSystemTags = (systemName: string): string[] => {
    const tagMap: Record<string, string[]> = {
      'Cardiovascular System': ['Cardiology', 'Emergency', 'ICU'],
      'Respiratory System': ['Pulmonology', 'Emergency', 'ICU'],
      'Gastroenterology': ['GI', 'Hepatology'],
      'Endocrine System': ['Endocrinology', 'DM'],
      'Nephrology': ['Nephrology', 'Emergency'],
      'Hematology': ['Hematology', 'Oncology'],
      'Infectious Diseases': ['ID', 'Emergency', 'ICU'],
      'Rheumatology': ['Rheumatology', 'Immunology'],
      'Neurology': ['Neurology', 'Emergency'],
      'Psychiatry': ['Psychiatry', 'Emergency'],
      'Emergency Medicine': ['Emergency', 'ICU', 'Trauma'],
      ...
      'Preventive Medicine': ['Prevention', 'Public Health'],
      'Radiology': ['Radiology', 'Imaging'],
      'Laboratory Medicine': ['Lab', 'Diagnostics'],
      'Pathology': ['Pathology'],
      'Internal Medicine Board Review': ['Board Review', 'High Yield']
    };
    
    return tagMap[systemName] || ['Medicine'];
  };
  ...
  const getDefaultSystems = () => [
    {
      ...
      icon: Heart,
      description: "Heart diseases, hypertension, arrhythmias, heart failure, coronary syndromes",
      progress: getSystemProgress(user.id, "Cardiovascular System"), // 🔒 REAL PROGRESS DATA
      totalQuestions: 156,
      lastStudied: formatLastStudied(getSystemLastStudied(user.id, "Cardiovascular System")), // 🔒 REAL PROGRESS DATA
      difficulty: "High Yield",
      tags: ["Cardiology", "Emergency", "ICU"]
    },
    {
      name: "Respiratory System", 
      icon: Activity,
      description: "Asthma, COPD, pneumonia, pulmonary embolism, respiratory failure",
      progress: getSystemProgress(user.id, "Respiratory System"), // 🔒 REAL PROGRESS DATA
      totalQuestions: 142,
      lastStudied: formatLastStudied(getSystemLastStudied(user.id, "Respiratory System")), // 🔒 REAL PROGRESS DATA
      difficulty: "High Yield",
      tags: ["Pulmonology", "Emergency", "ICU"]
    },
    {
      name: "Gastroenterology",
      icon: Plus,
      description: "GI bleeding, IBD, hepatitis, peptic ulcer disease, pancreatitis",
      progress: getSystemProgress(user.id, "Gastroenterology"), // 🔒 REAL PROGRESS DATA
      totalQuestions: 134,
      lastStudied: formatLastStudied(getSystemLastStudied(user.id, "Gastroenterology")), // 🔒 REAL PROGRESS DATA
  ...
  const handleStartPractice = (systemName?: string) => {
    ...
    const system = systemName || selectedSystem || "Cardiovascular System";
  ...
  const handleMockExamClick = (examName: string, examSet?: MockExamSet) => {
    ...
    const examConfig = examSet ? {
      ...
      examTitle: `${examSet.name} - FCPS Medicine Examination`
    } : {
      ...
      passingScore: 60,
      examTitle: `${examName} - FCPS Medicine Examination`
    };
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50">
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Welcome back, Dr. {(user.name || '').split(' ')[1] || user.name || 'User'}!</h1>
                <p className="text-gray-600">Internal Medicine Dashboard</p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-emerald-600">{realUserAccuracy}%</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-emerald-600">{realQuestionsCount.toLocaleString()}</p>
                </div>
                <div className="w-12 h-12 bg-emerald-100 rounded-lg flex items-center justify-center">
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-emerald-600">{realStudyStreak} {realStudyStreak === 1 ? 'day' : 'days'}</p>
                </div>
                <div className="w-12 h-12 bg-emerald-100 rounded-lg flex items-center justify-center">
        ...
        {showContinueSection && sessionSummary?.hasSession && (
          <Card className="border-emerald-200 bg-gradient-to-r from-emerald-500 to-teal-600 text-white mb-8">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      ...
                      className="text-emerald-200 hover:text-white text-sm opacity-75 hover:opacity-100 transition-opacity"
                  ...
                  <div className="w-full bg-emerald-400/30 rounded-full h-2 mb-4">
                    <div 
                    ...
                    return (
                      ...
                      >
                        <CardContent className="p-4">
                          ...
                          <p className="text-sm font-medium text-emerald-700">{config.questionCount} questions</p>
                        </CardContent>
                      </Card>
                    );
                  })}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Button
            variant="outline"
            className="h-20 flex-col space-y-2 border-emerald-200 hover:bg-emerald-50 hover:border-emerald-300"
        ...
        <div className="mb-8">
          ...
          {isLoadingSystems ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <Card key={i} className="border-emerald-200 bg-white/80 backdrop-blur-sm animate-pulse">
                  <CardContent className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {medicalSystems.map((system, index) => {
                ...
                return (
                  <Card 
                    key={index}
                    className={`border-emerald-200 bg-white/80 backdrop-blur-sm transition-all group ${
        ...
        <Card className="border-emerald-200 bg-white/80 backdrop-blur-sm mb-8">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-emerald-700">Mock Examinations</CardTitle>
                  ...
                  return (
                    <Card 
                      ...
                      className={`relative border-2 transition-all duration-200 ${
                        unlocked 
                          ? 'border-emerald-200 hover:border-emerald-300 hover:shadow-lg cursor-pointer' 
