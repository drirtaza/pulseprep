...
export function Footer({ onNavigate, onNavigateToSection }: FooterProps) {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToContentChanges((updatedSettings) => {
      ...
      console.log('📝 Content updated in Footer:', updatedSettings.version);
    });
  ...
  const quickLinks = [
    { label: 'Specialties', action: () => onNavigateToSection?.() },
    { label: 'Features', action: () => onNavigateToSection?.() },
    { label: 'Pricing', action: () => onNavigateToSection?.() },
    { label: 'About Us', action: () => onNavigate?.('about') }
  ];

  const supportLinks = [
    { label: 'Contact Us', action: () => onNavigate?.('contact') },
    { label: 'FAQ', action: () => onNavigateToSection?.() },
    { label: 'Help Center', action: () => alert('Help Center - Coming Soon') },
    { label: 'Privacy Policy', action: () => alert('Privacy Policy - Coming Soon') }
  ];

  const accountLinks = [
    { label: 'Sign In', action: () => onNavigate?.('sign-in') },
    { label: 'Create Account', action: () => onNavigate?.('sign-up') },
    { label: 'Terms of Service', action: () => alert('Terms of Service - Coming Soon') },
    { label: 'Student Discounts', action: () => alert('Student Discounts - Coming Soon') }
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xl text-white">{contentSettings.platformName || 'PulsePrep'}</span>
            </div>
            <p className="text-slate-400 text-sm leading-relaxed">
              {contentSettings.description || 'Empowering medical professionals with AI-powered exam preparation and comprehensive study tools.'}
            </p>
            <div className="flex space-x-4">
              {socialLinks.map((social, index) => {
                ...
                return (
                  <button
                    key={index}
                    onClick={() => alert(`${social.label} - Coming Soon`)}
          ...
          {/* Account */}
          <div>
            <h3 className="text-lg mb-4 text-white">Account</h3>
            <ul className="space-y-3">
